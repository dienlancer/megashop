...
<form class="form-horizontal" role="form">	
	<div class="portlet light bordered">
		...
		<div class="portlet-body">		
			<table class="table table-striped table-bordered table-hover table-checkable order-column" id="tbl-setting-system">
				<thead>
					<tr>
...
<script type="text/javascript" language="javascript">	
	function getList() {  	
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {  				
				...
				vSettingSystemTable.clear().draw();
				vSettingSystemTable.rows.add(data).draw();
				spinner.hide();
			},
	...
	function checkWithList(this_checkbox){
		var dr = vSettingSystemTable.row( $(this_checkbox).closest('tr') ).data();       		
		...
		vSettingSystemTable.row( $(this_checkbox).closest('tr') ).data(dr);
	}	
	function changeStatus(id,status){		
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {   							                              				
				...
				vSettingSystemTable.clear().draw();
				vSettingSystemTable.rows.add(data.data).draw();
				spinner.hide();
			},
			beforeSend  : function(jqXHR,setting){
				spinner.show();
	...
	function updateStatus(status){		
		var token 	= 	$('input[name="_token"]').val();   
		var dt 		= 	vSettingSystemTable.data();
		var str_id	=	"";		
