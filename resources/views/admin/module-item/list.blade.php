...
<form class="form-horizontal" role="form">	
	<div class="portlet light bordered">
		...
		<div class="portlet-body">		
			<table class="table table-striped table-bordered table-hover table-checkable order-column" id="tbl-module-item">
				<thead>
					<tr>
...
<script type="text/javascript" language="javascript">	
	...
	function getList() {  	
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {  				
				...
				vModuleItemTable.clear().draw();
				vModuleItemTable.rows.add(data).draw();
				spinner.hide();
			},
	...
	function checkWithList(this_checkbox){
		var dr = vModuleItemTable.row( $(this_checkbox).closest('tr') ).data();       		
		...
		vModuleItemTable.row( $(this_checkbox).closest('tr') ).data(dr);
	}	
	function changeStatus(id,status){		
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {   							                              				
				...
				vModuleItemTable.clear().draw();
				vModuleItemTable.rows.add(data.data).draw();
				spinner.hide();
			},
			beforeSend  : function(jqXHR,setting){
				spinner.show();
